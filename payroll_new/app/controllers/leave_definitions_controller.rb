class LeaveDefinitionsController < ApplicationController
  ...
  def index
    @leave_definitions = LeaveDefinition.all
  ...
  def create
    ...
    respond_to do |format|
      if @leave_definition.save
        format.html { redirect_to @leave_definition, :notice => 'Leave definition was successfully created.' }
        format.json { render :json => @leave_definition, :status => :created, :location => @leave_definition }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @leave_definition.update_attributes(params[:leave_definition])
        format.html { redirect_to @leave_definition, :notice => 'Leave definition was successfully updated.' }
        format.json { head :ok }
      else
